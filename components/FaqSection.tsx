    ...
    return (
        <div className="border-b border-gray-200 py-6">
            <button onClick={onClick} className="w-full flex justify-between items-center text-left gap-4">
                <h3 className={`text-lg font-semibold transition-colors ${isOpen ? 'text-brand-dark-blue' : 'text-gray-700'}`}>{faq.question}</h3>
                <div className={`text-xl flex-shrink-0 ${isOpen ? 'text-brand-dark-blue' : 'text-gray-500'}`}>
...
const FaqSection: React.FC<FaqSectionProps> = ({ subtitle, title, faqs, image, stat1, stat2 }) => {
        ...
        >
            <div className="container mx-auto px-4 lg:px-8">
                <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                    <div>
                        {subtitle && <p className="font-semibold text-gray-500 tracking-widest mb-2">{subtitle}</p>}
                        <h2 className="text-4xl lg:text-5xl font-extrabold text-brand-dark-blue leading-tight mb-8">{title}</h2>
                        <div>
                            {faqs.map((faq, index) => (
                                <FaqItem
                                    key={index}

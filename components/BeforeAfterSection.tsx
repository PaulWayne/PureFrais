...
const BeforeAfterCard: React.FC<{ item: DemoItem }> = ({ item }) => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col lg:flex-row">
            <div className="lg:w-1/2 p-8">
                <h3 className="text-2xl font-bold text-brand-dark-blue">{item.title}</h3>
                <p className="text-sm font-semibold text-gray-500 mt-1 mb-4">{item.date}</p>
                <p className="text-gray-600 mb-4">{item.description}</p>
                <div className="bg-slate-100 p-3 rounded-md text-sm">
                    <span className="font-bold text-brand-dark-blue">Surface:</span>
                    <span className="text-gray-700 ml-2">{item.surface}</span>
                </div>
            </div>

...
const CleanlinessCard: React.FC<{ icon: string; title: string, delay: number }> = ({ icon, title, delay }) => (
    <motion.div
    ...
    >
        ...
        <a href="#" className="font-semibold text-brand-dark-blue hover:text-brand-teal transition-colors">
            <i className="fas fa-arrow-right"></i>
        </a>
    </motion.div>
);

const CleanlinessSection: React.FC = () => {
    return (
                ...
                <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                    <div>
                        ...
                        <h2 className="text-4xl lg:text-5xl font-extrabold text-brand-dark-blue leading-tight mb-6">
                            La propreté pour la maison et le bureau
                        </h2>
                    </div>
                    <div>
                        <p className="text-gray-600">
                           Nous offrons une gamme complète de services pour répondre à tous vos besoins de propreté. Notre expertise garantit des résultats impeccables, que ce soit pour votre espace personnel ou professionnel.
                        </p>
                    </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mt-16 border-t border-gray-200 pt-16">
                    {cleanlinessServices.map((service, index) => (
                        <div key={index} className="relative">
                            <CleanlinessCard icon={service.icon} title={service.title} delay={index * 0.15} />

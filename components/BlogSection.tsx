...
const BlogPostCard: React.FC<{ post: typeof blogPosts[0] }> = ({ post }) => {
    return (
        <motion.div variants={itemVariants} className="bg-white rounded-lg shadow-md overflow-hidden group flex flex-col h-full">
            <Link href="/blog" className="block h-full flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-grow">
                    <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
                        <span className="font-semibold bg-brand-green/20 text-brand-dark-blue px-2 py-1 rounded-full text-xs uppercase tracking-wider">{post.category}</span>
                        <span>{post.date}</span>
                    </div>
                    ...
                    <div className="mt-auto">
                        <span className="font-semibold text-brand-dark-blue group-hover:text-brand-teal transition-colors">
                            Lire la suite <i className="fas fa-arrow-right ml-1 transition-transform duration-300 group-hover:translate-x-1"></i>
                        </span>
                    </div>
                </div>
            </Link>
        ...
        >
            <div className="container mx-auto px-4 lg:px-8">
                <div className="text-center mb-12">
                    ...
                    <h2 className="text-4xl lg:text-5xl font-extrabold text-brand-dark-blue">Dernières nouvelles & Articles</h2>
                </div>
